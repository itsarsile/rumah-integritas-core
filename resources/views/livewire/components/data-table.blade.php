...
<div class="card bg-white w-full border border-base-200 rounded-2xl">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 p-4">
        ...
        <div class="relative flex-1 max-w-md">
            ...
            <input 
                ...
                type="text" 
                class="block w-full max-w-xs pl-10 pr-3 py-2 border border-base-300 rounded-lg leading-5 bg-white placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" 
                placeholder="Search..."
            >
            @if($search)
                <button 
        ...
        <div class="flex items-center space-x-4">
            ...
            @if(!empty($filterableColumns))
                <button 
                    ...
                    class="inline-flex items-center px-3 py-2 border border-base-300 text-sm leading-4 font-medium rounded-lg text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                >
            ...
            <div class="flex items-center space-x-2">
                ...
                <select wire:model.live="perPage" class="select border border-base-300 bg-white">
    ...
    @if($showFilters && !empty($filterableColumns))
        <div class="px-4 pb-4">
            <div class="p-4 bg-white border border-base-300 rounded-lg">
                    ...
                    @foreach($filterableColumns as $filter)
                        <div class="space-y-1">
                            ...
                            @if($filter['type'] === 'select')
                                <select wire:model.live="filters.{{ $filter['key'] }}" class="select border border-base-300 bg-white">
                            ...
                            @elseif($filter['type'] === 'date')
                                <input wire:model.live="filters.{{ $filter['key'] }}" type="date" class="block w-full border border-base-300 rounded-lg sm:text-sm">
                            
                            @elseif($filter['type'] === 'date_range')
                                <div class="flex space-x-2">
                                    <input wire:model.live="filters.{{ $filter['key'] }}.from" type="date" placeholder="From" class="input border border-base-300 rounded-lg bg-white">
                                    <input wire:model.live="filters.{{ $filter['key'] }}.to" type="date" placeholder="To" class="input border border-base-300 rounded-lg bg-white">
                                </div>
    ...
    {{-- <!-- Loading State -->
    <div wire:loading class="flex justify-center items-center px-4 w-full">
    ...
    <!-- Table -->
    <div class="px-4">
        <div class="overflow-x-auto border border-base-200 rounded-2xl overflow-hidden" wire:loading.class="opacity-50">
    ...
    <!-- Results Info -->
    <div class=" flex items-center justify-between text-sm text-gray-700 p-4">
        <div>
