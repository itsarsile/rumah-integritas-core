<div class="card bg-white w-full border border-base-200 rounded-2xl">
    <div class="p-6 border-b border-base-200">
        ...
        <span class="text-sm font-light">Daftar semua pengguna</span>
    </div>    
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 p-4">
        <div class="border border-base-200 rounded-2xl p-4 space-y-4">
            ...
            <div class="flex items-center justify-between text-primary">
                <p class="text-4xl font-medium">{{ $totalUsers }}</p>               
        ...
        <div class="border border-base-200 rounded-2xl p-4 space-y-4">
            ...
            <div class="flex items-center justify-between text-success">
                <p class="text-4xl font-medium">{{ $activeUsers }}</p>   
        ...
        <div class="border border-base-200 rounded-2xl p-4 space-y-4">
            ...
            <div class="flex items-center justify-between text-warning">
                <p class="text-4xl font-medium">{{ $pendingUsers }}</p>  
                <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="border border-base-200 rounded-2xl p-4 space-y-4">
            <div class="stat-title">Pengguna Nonaktif</div>
            <div class="flex items-center justify-between text-error">
                <p class="text-4xl font-medium">{{ $inactiveUsers }}</p>               
    ...
    {{-- Filters and Search --}}
    <div class="flex flex-col lg:flex-row gap-4 justify-between p-4 pb-0 border-t border-base-200">
        {{-- Search Input --}}
        ...
        <div class="flex gap-4">
            ...
            <div class="form-control">
                <select class="select select-bordered border border-base-200 bg-white" wire:model.live="statusFilter">
            ...
            {{-- Role Filter --}}
            <div class="form-control">
                <select class="select select-bordered border border-base-200 bg-white" wire:model.live="roleFilter">
            ...
            <button wire:click="openCreateModal" class="btn btn-primary shadow-none">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                </svg>
                Add New User
    ...
    {{-- Users Table --}}
    <div class="p-4">
        <div class="card rounded-2xl border border-base-200">
                            ...
                            @forelse($users as $user)
                                <tr wire:key="{{ $user->id }}">
                                    ...
                                    <td>
                                        ...
                                        @else
                                            <div class="badge badge-error badge-xs">Unverified</div>
                                        @endif
                                    </td>
                                    <td>
                                        <div class="px-2 py-1 text-xs border border-base-300 rounded-full w-fit">{{ $user->roles->isEmpty() ? 'None' : ucfirst($user->roles->pluck('name')->implode(', ')) }}</div>
                                    </td>
                                    <td>
                                        @switch($user->status)
                                            @case('active')
                                                <div class="px-2 py-1 text-xs border border-green-300 bg-green-200 text-green-700 rounded-full w-fit">Active</div>
                                                @break
                                            @case('inactive')
                                                <div class="px-2 py-1 text-xs border border-red-300 bg-red-200 text-red-700 rounded-full w-fit">Inactive</div>
                                                @break
                                            @case('pending')
                                                <div class="px-2 py-1 text-xs border border-yellow-300 bg-yellow-200 text-yellow-700 rounded-full w-fit">Pending</div>
                                                @break
                                            @case('suspended')
                                                <div class="px-2 py-1 text-xs border border-gray-300 bg-gray-200 text-gray-700 rounded-full w-fit">Suspended</div>
                                                @break
                                            @default
                                                <div class="px-2 py-1 text-xs border border-gray-300 bg-gray-200 text-gray-700 rounded-full w-fit">Unknown</div>
                                        @endswitch
                                    </td>
                                    <td>
        ...
        <div class="modal-box max-w-2xl bg-base-100 shadow-2xl border border-base-300 rounded-3xl">
            ...
            <form wire:submit.prevent="createUser" class="space-y-2">
                ...
                <div class="flex gap-3 pt-4 border-t border-base-200">
                    <button type="submit"
                        class="btn btn-primary flex-1 gap-2transition-all duration-200">
    ...
    x-cloak>
        <div class="modal-box max-w-2xl bg-base-100 shadow-2xl border border-base-300 rounded-3xl">
            ...
            <form wire:submit.prevent="updateUser" class="space-y-6">
                ...
                <fieldset class="fieldset">
                    <legend class="fieldset-legend">
                        <span class="label-text font-semibold text-base-content text-sm">Name</span>
                    </legend>
                    <input type="text" placeholder="Enter full name..."
                        class="w-full input input-bordered bg-white border-1 focus:border-warning hover:border-warning/50"
                        wire:model="name">
                    @error('name')
                        <label class="label">
                ...
                <fieldset class="fieldset">
                    <legend class="fieldset-legend">
                        <span class="label-text font-semibold text-base-content text-sm">Email</span>
                    </legend>
                    <input type="email" placeholder="Enter email address..."
                        class="w-full input input-bordered bg-white border-1 focus:border-info hover:border-info/50"
                        wire:model="email">
                    @error('email')
                        <label class="label">
                            <span class="label-text-alt text-error">{{ $message }}</span>
                ...
                <fieldset class="fieldset">
                    <legend class="fieldset-legend">
                        <span class="label-text font-semibold text-base-content text-sm">Role</span>
                    </legend>
                    <select
                        class="w-full select select-bordered bg-white border-1 focus:border-accent hover:border-accent/50"
                        wire:model="selectedRoleId">
                        @foreach($roles as $role)
                            <option value="{{ $role->id }}">{{ $role->name }}</option>
                        @endforeach
                ...
                <fieldset class="fieldset">
                    <legend class="fieldset-legend">
                        <span class="label-text font-semibold text-base-content text-sm">Status</span>
                    </legend>
                    <select
                        class="w-full select select-bordered border-1 bg-white focus:border-secondary hover:border-secondary/50"
                        wire:model="status">
                        <option value="active">✅ Active</option>
                        <option value="inactive">🚫 Inactive</option>
                ...
                <fieldset class="fieldset">
                    <legend class="fieldset-legend">
                        <span class="label-text font-semibold text-base-content text-sm">New Password (optional)</span>
                    </legend>
                    <input type="password" placeholder="Enter new password..."
                        class="w-full input input-bordered bg-white border-1 focus:border-primary hover:border-primary/50"
                        wire:model="password">
                    @error('password')
                        <label class="label">
                            <span class="label-text-alt text-error">{{ $message }}</span>
                ...
                <div class="flex gap-3 pt-4 border-t border-base-200">
                    <button type="submit"
                        class="btn btn-warning flex-1 gap-2 transition-all duration-200">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M5 13l4 4L19 7" />
                        </svg>
