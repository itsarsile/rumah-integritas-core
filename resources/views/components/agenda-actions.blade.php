<div class="flex items-center gap-1">
    <a class="size-7 flex items-center justify-center rounded-lg hover:bg-neutral-200/70" href="{{ route('dashboard.agenda.show', $agenda->id) }}" title="Lihat">
    ...
    @if($canMark)
        <button type="button" class="size-7 flex items-center justify-center rounded-lg hover:bg-neutral-200/70" title="Tandai kalender"
            onclick="(function(){if(!confirm('Tambah/hapus tanda kalender untuk tanggal ini?')) return; fetch('{{ route('dashboard.calendar.toggle') }}',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':'{{ csrf_token() }}'},credentials:'same-origin',body:JSON.stringify({module:'agenda',date:'{{ optional($agenda->date ?? $agenda->created_at)->format('Y-m-d') }}',division_id: {{ (int)(auth()->user()->division_id) }},entity_id: {{ (int)$agenda->id }},entity_type: 'agenda' })}).then(r=>r.json()).then(()=>{ window.dispatchEvent(new CustomEvent('calendar-mark-changed')); window.dispatchEvent(new CustomEvent('ri-toast',{detail:{text:'Kalender diperbarui'}})); }).catch(()=>{});})();">
